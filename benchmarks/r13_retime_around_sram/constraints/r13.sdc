# r13 -- retime_around_sram: ASIC constraints (?)

if {![info exists ::CLK_PERIOD]} { set ::CLK_PERIOD 0.957 }
...
set_input_delay  [expr {0.15 * $::CLK_PERIOD}] -clock clk [get_ports {rst_n addr wdata we}]
set_output_delay [expr {0.15 * $::CLK_PERIOD}] -clock clk [get_ports {dout}]

set_driving_cell -lib_cell BUF_X4 -pin Z \
    [get_ports {rst_n addr wdata we}]
